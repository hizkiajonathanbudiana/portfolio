...
const PhilosophyJourneyComponent = () => {
  const sectionRef = useRef(null);
  const trackRef = useRef(null);
  ...
  return (
    ...
    >
      <div
        ref={trackRef}
        className="h-full flex items-center px-[10vw] gpu-layer"
      >
        <div className="flex-shrink-0 w-[80vw] md:w-[50vw] pr-12 philosophy-item gpu-layer">
          <h2 className="text-5xl md:text-7xl font-bold">Code is Craft.</h2>
          <p className="text-lg text-gray-300 mt-4">
            My development philosophy is rooted in established principles to
            ensure clean, efficient, and future-proof code.
        ...
        {philosophyData.map((p) => (
          <div
            key={p.id}
            className="flex-shrink-0 w-[70vw] md:w-[40vw] p-8 philosophy-item gpu-layer"
          >
            <h3 className="text-4xl md:text-6xl font-semibold text-background">
              {p.title}
            </h3>
...
const PhilosophyJourney = React.memo(PhilosophyJourneyComponent);
export default PhilosophyJourney;
